...
def generate_username() -> str:
    ...
    random_chars = "".join(
        random.choice(string.ascii_letters + string.digits, k=remaining_length)
    )
...
class UserManager(DjangoUserManager):
    def _create_user(self, email:str, password:str, **extra_fields: Any):
        if not email:
            raise ValueError(_("An emaul address must be provided."))
    ...
    def create_superuser(self, email:str, password:Optional[str]=None, **extra_fields):
        ...
        if extra_fields.get("is_supperuser") is not True:
            raise ValueError(_("Superuser must have is_supperuser=True"))
