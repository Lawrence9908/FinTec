...
def send_otp_email(email, otp):
    ...
    email.attach_alternative(subject, plain_email, from_email, recipient_list)
    try:
        email.send()
...
def send_account_locked_email(self):
    ...
    html_email = render_to_string("email/account_locked_email.html", context)
    plain_email= strip_tags(html_email)
    email  = EmailMultiAlternatives(subject, plain_email, from_email, recipient_list)
    email.attach_alternative(html_email, "text/html")
    try:
        email.send()
        logger.info(f"OTP email sent successfully to: {self.email}")
    except Exception as e:
        logger.error(f"Failed to  send OTP email to {self.email}: Error: {str(e)}")